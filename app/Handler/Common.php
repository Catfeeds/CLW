...
class Common
{
    ...
    // 工单编号
    public static function identifier($lastData)
    {
        if (empty($lastData->gd_identifier)) {
            return $identifier = 'gd'.date('ymd',time()).'0001';
        } else {
            $lastIdentifier = $lastData->gd_identifier;
        }
        ...
        if (empty($lastIdentifier)) {
            ...
            if (date('Y-m-d',strtotime('20'.$date) + (3600 * 24)) == date('Y-m-d',time())) {
                ...
                return $identifier = 'gd'.date('ymd',time()).$temp;
            }
        }
    }
